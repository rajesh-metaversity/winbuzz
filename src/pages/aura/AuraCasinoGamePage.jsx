...
const AuraCasinoGamePage = () => {
    ...
    const url = 'https://admin-api-banners-new.s3.ap-south-1.amazonaws.com/wolf.json';
    ...
    return (
        ...
        <>
            {<Grid container sx={{ justifyContent: 'space-between', paddingInline: '2px', gap: '2px' }}>
                ...
                {cards?.map((games, index) => (
                    <Grid key={index} xs={5.9} md={2.9} item sx={{ cursor: 'pointer', overflow: 'hidden', }} onClick={() => handleClickOpen(games?.gameId, games?.gameName)}>
                        <img src={games?.imageUrl} alt='' style={{ objectFit: 'contain', width: '100%', height: '100%', }} />
                    </Grid>
                ))}
            </Grid>}
            ...
            <ModalComponent
                Elememt={
                    <CasinoRuleModalContent handleClose={handleClose}
                        gameName={gameName}
                        gameId={casinoId}
                    />
                }
