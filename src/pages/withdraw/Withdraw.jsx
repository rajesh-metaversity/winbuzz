...
const Withdraw = () => {
  ...
  const [
    triger,
    { data: accountDetails, error: bankError, isLoading: bankDetailsLoading },
  ] = useBankAccountMutation();
  ...
  useEffect(() => {
    try {
      if (withdrawBalance) {
        showSuccessToast(withdrawBalance?.message);
      } else {
    ...
    <>
      <ModalComponent
        Elememt={
          <WithdrawModal
            triger={triger}
            withdrawTypeHandler={withdrawTypeHandler}
            withdrawDetails={withdrawDetails}
            setWithdrawDetails={setWithdrawDetails}
            setSaveBankModal={setSaveBankModal}
          />
        }
        open={saveBankModal}
        setOpen={setSaveBankModal}
      />
            ...
            >
              {paymentImage?.data?.map((imgdata, index) => (
                <Tab
                  ...
                  sx={{
                    border: `${value === index ? "1px solid #b6842d" : 0}`,
                  }}
                  label={
                    <div
                      onClick={() =>
