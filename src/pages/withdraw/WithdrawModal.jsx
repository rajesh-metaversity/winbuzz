...
const WithdrawModal = ({ withdrawDetails, setWithdrawDetails, setSaveBankModal, triger, withdrawTypeHandler }) => {
	const [trigger, { data: saveBankDetails, error: saveBankError, isLoading: saveBankDetailsLoading }] = useSaveBankAccountMutation();
	...
	useEffect(() => {
        if (saveBankDetails?.status) {
            toast.success(saveBankDetails?.message)
	...
	return (
        <div className='save-details-modal'>
            <div className='save-details-heading'><span>Save Details</span><span onClick={() => setSaveBankModal(false) }><CloseIcon /></span></div>
