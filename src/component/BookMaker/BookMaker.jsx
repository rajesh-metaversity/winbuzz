...
const BookMaker = ({ data, ip, setMinMax, prevOdds, minMax, PnlOdds, favData, handleFavDel, handleFavSec }) => {
	...
	const handleBackBet = (marketId, matchName, sid, odds, priceValue, isBack, isFancy, fullmatchName, min, max) => {
		setMinMax({
			minBet: min,
			maxBet: max
		});
	...
	const createMarketID = data && favData?.find(pnl => pnl?.marketId === data[0]?.mid);
	return (
		<>
			<MainDiv>
				<GridContainer container>
					<Grid item xs={7} md={4}>
						<PolygonStrip>
							{createMarketID ? (
								<StarIcon onClick={() => handleFavDel(data && data[0]?.mid)} fontSize="medium" sx={{ color: '#ffcf03' }} />
							) : (
					...
					{data?.map((dataBook, id) => {
						...
						return (
							<Grid
									...
									{PnlOdds?.map((item, id) => {
										...
										return (
											<div className="sub_title" key={id}>
												<span className={`font_normal ${oddsPnl[dataBook.sid] <= 0 ? 'text_danger' : 'text_success'}`}>
											...
											<>
												<Grid item xs={6}>
													<Grid container gap={{ md: '1%', xs: '2%' }} sx={{ justifyContent: 'center' }}>
														<BackGrid className="back" display={{ xs: 'none', md: 'block' }} item md={3.9} xs={0}>
														...
														<BackGrid
															...
															md={3.9}
															xs={12}
															className={(dataBook && dataBook?.b1 > prevOdds[id]?.b1 ? 'odds-up-color ' : '') + 'back'}
															onClick={() => {
																handleBackBet(
																	dataBook?.mid,
																	dataBook?.nation,
															...
															<BetTypoSpan>{dataBook?.bs1.length ? dataBook?.bs1 : '0'}</BetTypoSpan>
														</BackGrid>
													</Grid>
												</Grid>
												<Grid item xs={6}>
													<Grid container gap={{ md: '1%', xs: '2%' }} sx={{ justifyContent: 'center' }}>
														<LayGrid
															className={(dataBook?.l1 > prevOdds[id].l1 ? 'odds-down-color' : '') + 'lay'}
															item
															md={3.9}
															xs={12}
															onClick={() => {
															...
															<BetTypoSpan>{dataBook?.ls1.length ? dataBook?.ls1 : '0'}</BetTypoSpan>
														</LayGrid>
														<LayGrid className="lay" display={{ xs: 'none', md: 'block' }} item md={3.9} xs={0}>
															<BetTypoPara></BetTypoPara>
															<BetTypoSpan></BetTypoSpan>
														</LayGrid>
														<LayGrid className="lay" display={{ xs: 'none', md: 'block' }} item md={3.9} xs={0}>
								...
								{dataBook?.sid == selectionids && <MobileBetPlaceModal minMax={minMax} />}
							</Grid>
						);
					})}
				</GridContainer>
