    ...
    <>
      <div className="my_bets-cont betslip">
        ...
        <div className="my-bets-module-container">
          ...
          {betlistbymatch?.data &&
            Object.keys(betlistbymatch?.data)?.map((matchdatakeys) =>
              betlistbymatch?.data[matchdatakeys]?.map((betlistdata, id) => (
                <Grid
                  container
                  key={id}
                  sx={{
                    backgroundColor: betlistdata?.back ? "#A5D9FE" : "#F8D0CE",
                    mb: 0.1,
                    py: 0.5,
                    px: 0.8,
                  }}
                >
                  <Grid item xs={8}>
                    <Typography component="p">{betlistdata.nation}</Typography>
                  </Grid>
                  <Grid item xs={2}>
                    <Typography component="p">{betlistdata.rate}</Typography>
                  </Grid>
                  <Grid item xs={2}>
                    <Typography component="p">{betlistdata.amount}</Typography>
                  </Grid>
                </Grid>
              ))
            )}
